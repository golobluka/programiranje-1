...
let is_full_row (prvo, drugo, tretje) =
  if prvo <> None && drugo <> None && tretje <> None then true
  else false
...
let winner_of_triple (prvi, drugi, tretji) = 
  let sez = [prvi; drugi; tretji] in
    if (List.for_all (fun x -> if Some X = x then true else false) sez) = true then Some X
    else if (List.for_all (fun x -> if Some O = x then true else false) sez) = true then Some O
    else None
