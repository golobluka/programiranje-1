...
let rec height  = function
     | Empty -> 0
     | Node (l, x, d) -> max (height l) (height d) + 1
...
let rec is_bst = function
     ...
     | Node ( (Node(l', x', d') as l), x, Empty) -> x' < x && (is_bst l)
     | Node ( Empty, x, (Node (l', x', d') as d)) -> x < x' &&  is_bst d
     | Node ( (Node(l', x', d') as l), x, (Node(l'', x'', d'') as d)) -> x' < x && x < x'' && is_bst l && is_bst d 
...
let rec delete x = function
     ...
     | Node (l, y, d) as drevo -> 
               if x < y then
                    ...
                    let nas = succ drevo in
                    match nas with
                         | None -> l 
                         | Some nas -> Node(l, nas, delete nas d)
...
let rec dict_insert key value = function
     ...
     | Node (l, x, r) -> 
          if fst x > key then
               Node( dict_insert key value l, x, r)
          else if fst x < key then
               Node (l, x, dict_insert key value r)
          else
               Node (l, (key, value) , r)
...
let bst_of_list list = 
   let rec vstavi x = function
       ...
       | Node(l, y, r) -> (
            if y > x 
              then Node(vstavi x l, y, r)
            else if y < x 
              then Node(l, y, vstavi x r)
            else Node(l, y, r)
       )
   in
...
[*----------------------------------------------------------------------------*)
let rec bst_to_list = function
     | Empty -> []
     | Node(l,x,r) -> bst_to_list l @ [x] @ bst_to_list r

let tree_sort list = 
     let tree = bst_of_list list in
     let sorted_list = bst_to_list tree in
     sorted_list
...
let rec follow dir tree = 
     match dir, tree with 
     | _ , Empty -> None
     | [] , Node(_, x, _) -> Some x 
     | head :: tail , Node(l, x', r) ->(
          match head with
          |Left -> follow  tail l
          |Right -> follow  tail r
     )
...
let rec prune dir tree =
     match dir, tree with
     | [], _ -> Empty
     | _, Empty -> Empty
     | head :: tail, Node(l, x, r) ->
          match head with
          | Left -> Node(prune tail l, x, r)
          | Right -> Node(l, x, prune tail r)
