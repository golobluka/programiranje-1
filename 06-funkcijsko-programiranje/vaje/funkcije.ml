...
let rec reverse2 sez = 
  let rec aux star_sez nov_sez = 
    match star_sez with
    | [] -> nov_sez
    | x :: xs -> aux xs (x :: nov_sez)
  in
  aux sez []
...
let rec maptr f sez =
  let rec aux f sez1 sez2 =
    match sez1 with
    | glava :: rep -> aux f rep ((f glava) :: sez2)
    | [] -> sez2
  in
    reverse (aux f sez [])
...
let rec unzip_tlrec sez = 
  let rec aux sez' (sez1, sez2) = 
    match sez' with
    | [] -> reverse sez1, reverse sez2
    | (l1, l2) :: rep -> 
      aux rep (l1 :: sez1, l2 :: sez2)
  in
  (aux sez ([], []))
...
let rec loop f  x0 pogoj = 
  if pogoj x0 then loop f (f x0)  pogoj else x0
...
let rec first f defoult = function
  | [] -> defoult
  | x :: xs -> if f x then x else first f defoult xs
